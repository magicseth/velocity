...
enum ConductorWorkspaces {
    ...
    static func current(_ window: AXUIElement) -> [(ChatProject, AXUIElement)] {
        ...
        func walk(_ node: AXUIElement, depth: Int) {
            ...
            if role == "AXLink" {
                let value = Accessibility.attribute(node, kAXURLAttribute)
                let raw = (value as? URL)?.absoluteString ?? value as? String ?? ""
                let name = workspaceName(Accessibility.label(node))
            ...
            let descendants = Accessibility.children(node)
        ...
        func findSidebar(_ node: AXUIElement, depth: Int) -> AXUIElement? {
            ...
            AXUIElementSetMessagingTimeout(node, 0.05)
            let kind = Accessibility.attribute(node, kAXRoleAttribute) as? String ?? ""
            ...
            if nodes.contains(where: {
                (Accessibility.attribute($0, kAXRoleAttribute) as? String) == kAXButtonRole && Accessibility.label($0) == "Toggle left sidebar"
            }) { return node }
            for child in nodes {
        ...
        if let sidebar = findSidebar(window, depth: 0) { walk(sidebar, depth: 0) }
        return result
    }
